...
@dataclass
class Move:
    row: int; col: int; flips: List[Tuple[int,int]]
...
class Board:
    ...
    move_list: List[Tuple[int,int,int]] = field(default_factory=list)  # (color,r,c) pass=>(-1,-1)

    def __post_init__(self):
        if not self.grid:
            self.grid = [[EMPTY for _ in range(self.size)] for _ in range(self.size)]
            m = self.size//2
            self.grid[m-1][m-1] = WHITE; self.grid[m][m] = WHITE
            self.grid[m-1][m] = BLACK;   self.grid[m][m-1] = BLACK
    ...
    def legal_moves(self, color: Optional[int]=None) -> List[Move]:
        ...
        for r in range(self.size):
            for c in range(self.size):
                if self.grid[r][c] != EMPTY: continue
                flips: List[Tuple[int,int]] = []
                for dr,dc in DIRS:
                    rr,cc=r+dr,c+dc; line=[]
                    ...
                    if line and self.inside(rr,cc) and self.grid[rr][cc]==color:
                        flips.extend(line)
                if flips: out.append(Move(r,c,flips))
        return out

    def make_move(self, mv: Move):
        self.history.append((self.copy_grid(), self.to_move)); self.redo_stack.clear()
        r,c=mv.row,mv.col; self.grid[r][c]=self.to_move
        for rr,cc in mv.flips: self.grid[rr][cc]=self.to_move
        self.move_list.append((self.to_move,r,c))
        self.to_move = OPP[self.to_move]

    def pass_turn(self):
        self.history.append((self.copy_grid(), self.to_move)); self.redo_stack.clear()
        self.move_list.append((self.to_move,-1,-1)); self.to_move=OPP[self.to_move]

    def undo(self):
        if not self.history: return False
        self.redo_stack.append((self.copy_grid(), self.to_move))
        grid,tomove=self.history.pop(); self.grid=[row[:] for row in grid]; self.to_move=tomove
    ...
    def redo(self):
        ...
        grid,tomove=self.redo_stack.pop(); self.grid=[row[:] for row in grid]; self.to_move=tomove
        return True
    ...
    def serialize(self):
        return {"size":self.size, "grid":self.grid, "to_move":self.to_move, "move_list":self.move_list}

    @staticmethod
    def deserialize(obj:dict)->'Board':
        b=Board(size=obj.get("size",8)); b.grid=[row[:] for row in obj["grid"]]; b.to_move=obj["to_move"]
        b.move_list=[tuple(x) for x in obj.get("move_list",[])]; b.history.clear(); b.redo_stack.clear(); return b
...
PST_TEMPLATES = {
    ...
    10: [[150, -25, 25, 10, 5, 5, 10, 25, -25, 150],
    ...
    12: [[180, -30, 30, 15, 8, 5, 5, 8, 15, 30, -30, 180],
...
class AI:
    ...
    def get_pst(self, board_size: int):
        ...
        for r in range(board_size):
            ...
            for c in range(board_size):
                ...
                elif ((r == 0 or r == board_size-1) and (c == 1 or c == board_size-2)) or \
                     ((c == 0 or c == board_size-1) and (r == 1 or r == board_size-2)):
                    value = -40
                # Diagonal squares next to corners are also risky
                elif (r == 1 and c == 1) or (r == 1 and c == board_size-2) or \
                     (r == board_size-2 and c == 1) or (r == board_size-2 and c == board_size-2):
                    value = -20
    ...
    def evaluate(self, board: Board, color: int) -> int:
        ...
        if game_phase < 0.25:  # Early game - focus on mobility and position
            score = positional * 4 + mobility * 15 + corner_count * 100 - frontier_penalty * 2
        elif game_phase < 0.75:  # Mid game - balanced approach
            score = positional * 3 + mobility * 8 + corner_count * 150 + material * 5 - frontier_penalty
    ...
    def choose(self, board: Board, color: int) -> Optional[Move]:
        ...
        for _, move in scored_moves:
            ...
            score = -self.search(board_copy, adaptive_depth - 1, -beta, -alpha, OPP[color])
    ...
    def is_dangerous_square(self, r: int, c: int, size: int, board: Board) -> bool:
        ...
        for corner_r, corner_c in corners:
            ...
            if abs(r - corner_r) <= 1 and abs(c - corner_c) <= 1 and (r != corner_r or c != corner_c):
                return True
        return False
    
    def search(self, board: Board, depth: int, alpha: float, beta: float, color: int) -> float:
        ...
        if depth > 2:  # Only sort for deeper searches to save time
            scored_moves = [(self.quick_move_eval(board, move, color), move) for move in moves]
            scored_moves.sort(key=lambda x: x[0], reverse=True)
    ...
    def save(self):
        try:
            ...
            if self.stats:
                ...
                stats_dict['recent_results'] = [result.__dict__ for result in self.stats.recent_results]
...
THEMES={
    'classic': {
        ...
        'felt':(32,108,62), 'grid':(10,70,38), 'hud':(252,252,254), 'text':(40,40,45), 'accent':(70,130,235), 'danger':(235,70,70)
    },
    'ocean': {
        'name': 'ocean', 'display': 'Ocean Blue',
        'felt':(25,78,132), 'grid':(15,50,85), 'hud':(240,248,255), 'text':(30,50,80), 'accent':(50,150,255), 'danger':(255,100,100)
    },
    'sunset': {
        'name': 'sunset', 'display': 'Sunset Orange',
        'felt':(156,78,25), 'grid':(120,50,15), 'hud':(255,248,240), 'text':(80,40,20), 'accent':(255,120,50), 'danger':(220,70,70)
    },
    'midnight': {
        'name': 'midnight', 'display': 'Midnight Dark',
        'felt':(25,35,50), 'grid':(15,25,40), 'hud':(35,35,40), 'text':(220,220,230), 'accent':(120,180,255), 'danger':(255,120,120)
    },
    'forest': {
        'name': 'forest', 'display': 'Forest Green',
        'felt':(45,85,35), 'grid':(25,55,20), 'hud':(248,252,248), 'text':(30,60,25), 'accent':(80,160,70), 'danger':(200,80,80)
    }
}
WOOD=(70,45,30)
        ...
        for line in lines:
            ...
            if y >= content_y - line_height:
                if line.startswith("Move Quality:"):
                    ...
                    change = analysis.board_control_after - analysis.board_control_before
                    color = (100, 200, 100) if change > 0 else (200, 100, 100) if change < 0 else theme['text']
                else:
                    color = theme['text']
...
class PlayerStats:
    ...
    def add_game_result(self, result: GameResult):
        ...
        # Update averages
        self.average_score = (self.average_score * (self.games_played - 1) + result.final_score_human) / self.games_played
    ...
    def __init__(self):
        ...
        self.steps = [
            ...
            TutorialStep(
                ...
                board_highlight=[(0,1), (0,2), (1,0), (2,0), (0,5), (0,6), (5,0), (6,0)],
...
class GameplayAnalyzer:
    ...
    def analyze_move(self, board: Board, move: Move, player: int) -> MoveAnalysis:
        ...
        move_quality = self._evaluate_move_quality(r, c, pieces_captured, corner_moves, mobility_change, board.size)
    ...
    def _evaluate_move_quality(self, r: int, c: int, pieces_captured: int, corners: int, mobility: int, board_size: int) -> str:
        ...
        corner_adjacent = [
            ...
            (0,board_size-2), (1,board_size-1), (1,board_size-2),  # Top-right
            (board_size-2,0), (board_size-1,1), (board_size-2,1),  # Bottom-left
            (board_size-2,board_size-1), (board_size-1,board_size-2), (board_size-2,board_size-2)  # Bottom-right
        ]
...
class MenuSystem:
    """Clean menu system for the game"""
    ...
    def setup_menus(self):
        ...
        game_items = [
            ...
            MenuItem(f"Board Size: {self.game.board.size}x{self.game.board.size}", self.game.on_cycle_board_size),
        ...
        # AI menu
        ai_items = [
            MenuItem(f"Black: {'Computer' if self.game.settings.ai_black else 'Human'}", self.game.on_toggle_ai_black),
            MenuItem(f"White: {'Computer' if self.game.settings.ai_white else 'Human'}", self.game.on_toggle_ai_white),
            MenuItem(f"Computer Level: {self.game.settings.ai_depth}", self.game.on_cycle_depth),
        ]
        
        # View menu
        view_items = [
            MenuItem(f"Theme: {THEMES[self.game.settings.theme]['display']}", self.game.on_cycle_theme),
            MenuItem(f"Hints: {'On' if self.game.settings.hints else 'Off'}", self.game.on_toggle_hints),
            MenuItem(f"Sound: {'On' if self.game.settings.sound else 'Off'}", self.game.on_toggle_sound),
        ]
        
        # Help menu
        help_items = [
            MenuItem("Strategy Tutorial", self.game.on_show_tutorial),
            MenuItem("Game Analysis", self.game.on_show_game_analysis, enabled=self.game.board.game_over()),
    ...
    def draw_menu_bar(self, screen, theme):
        ...
        pg.draw.line(screen, border_color, (0, MENU_BAR_HEIGHT-1), (screen.get_width(), MENU_BAR_HEIGHT-1))
        ...
        for menu in self.menus:
            ...
            if is_hover:
                ...
                hover_color = tuple(min(255, c+10) if theme['name'] == 'midnight' else max(0, c-10) for c in menu_bg)
    ...
    def draw_dropdown(self, screen, theme):
        ...
        for item in menu.items:
            item_rect = pg.Rect(dropdown_rect.x + 5, y, dropdown_rect.width - 10, MENU_ITEM_HEIGHT)
            ...
            screen.blit(text, (item_rect.x + 8, item_rect.centery - text.get_height()//2))
...
class GameAnalysisDisplay:
    ...
    def generate_analysis(self) -> PostGameAnalysis:
        ...
        if self.game.ui.game_start_time is not None and self.game.ui.game_end_time is not None:
        ...
        for i, entry in enumerate(self.game.ui.move_history):
            ...
            if move_analysis.move_quality in ["Excellent", "Brilliant"]:
                critical_moments.append((i + 1, f"Excellent {move_analysis.move_quality.lower()} move"))
        ...
        suggestions = self._generate_suggestions(move_analyses, black_stats, white_stats)
    ...
    def _calculate_player_stats(self, moves: List[MoveAnalysis]):
        ...
        avg_mobility = sum(m.mobility_change for m in moves) / len(moves) if moves else 0
        avg_control = sum((m.board_control_after - m.board_control_before) for m in moves) / len(moves) if moves else 0
    ...
    def _classify_opening(self) -> str:
        ...
        first_moves = [(entry.row, entry.col) for entry in self.game.ui.move_history[:4]]
        ...
        edge_moves = sum(1 for r, c in first_moves 
    ...
    def _generate_suggestions(self, moves: List[MoveAnalysis], 
                            black_stats: dict, white_stats: dict) -> List[str]:
        ...
        if poor_moves / total_moves > 0.3:
            suggestions.append("Focus on strategic planning - consider mobility and board control")
        
        # Corner strategy
        total_corners = black_stats["corners_taken"] + white_stats["corners_taken"]
        if total_corners < 2:
            suggestions.append("Pay more attention to corner opportunities - they're worth many pieces")
        
        # Mobility awareness
        avg_mobility = (black_stats["avg_mobility_change"] + white_stats["avg_mobility_change"]) / 2
        if avg_mobility < 0:
            suggestions.append("Consider moves that preserve your future options (mobility)")
        
        # Opening improvement
        if len(moves) > 8:
            early_poor = sum(1 for m in moves[:8] if m.move_quality == "Poor")
            if early_poor >= 3:
                suggestions.append("Work on opening principles - control center, maintain mobility")
        
        # Endgame improvement  
        if len(moves) > 12:
            late_poor = sum(1 for m in moves[-8:] if m.move_quality == "Poor")
            if late_poor >= 3:
                suggestions.append("Practice endgame tactics - every piece counts in the final moves")
    ...
    def draw(self, screen, theme):
        ...
        # Close hint
        close_hint = self.small_font.render("Press 'G' or ESC to close", True, theme['text'])
        screen.blit(close_hint, (panel_x + panel_width - close_hint.get_width() - 20, panel_y + 15))
        ...
        # Scroll indicators
        if start_line > 0:
            pg.draw.polygon(screen, theme['accent'], 
    ...
    def _format_analysis_text(self, analysis: PostGameAnalysis) -> List[str]:
        ...
        lines.append(f"Final score: Black {analysis.final_score[0]} - {analysis.final_score[1]} White")
...
class Game:
    def __init__(self, board:Board, settings:Settings):
        pg.init(); pg.display.set_caption("Reversi Deluxe")
        self.screen=pg.display.set_mode((W,H), pg.RESIZABLE)
        self.clock=pg.time.Clock()
        self.font=pg.font.SysFont("Arial", 18)
        self.big_font=pg.font.SysFont("Arial", 24)
        self.board=board; self.settings=settings; self.ui=UIState()
    ...
    def ensure_icon(self):
        if os.path.exists(ICON_PNG): return
        try:
            # Simple generated icon
            import PIL.Image, PIL.ImageDraw
            img=PIL.Image.new('RGBA',(256,256),(0,0,0,0))
            d=PIL.ImageDraw.Draw(img)
            d.rectangle([16,16,240,240], fill=(29,102,57), outline=(10,60,30), width=8)
            d.ellipse([60,60,196,196], fill=(240,240,240), outline=(0,0,0), width=6)
    ...
    def layout(self):
        ...
        size = min(available_width, available_height); size = max(360, size)
        bx = (w - size) // 2; by = HUD_HEIGHT + (available_height - size) // 2
    ...
    def draw_hud(self, hud_rect, theme):
        ...
        if not self.board.game_over():
            ...
            self.screen.blit(turn_surface, (start_x, status_y + (status_height - turn_surface.get_height()) // 2))
            self.screen.blit(score_surface, (start_x + turn_surface.get_width() + 20, status_y + (status_height - score_surface.get_height()) // 2))
            ...
            analysis_hint = self.font.render("Press 'G' for game analysis", True, theme['text'])
        ...
        if self.settings.ai_black and self.settings.ai_white and not self.board.game_over():
            auto_font = pg.font.SysFont("Arial", 11, bold=True)
            auto_text = auto_font.render("AUTO", True, tuple(c//2 for c in theme['accent']))
    ...
    def draw_board(self, board_rect, cell, theme):
        ...
        pg.draw.rect(self.screen, theme['felt'], board_rect, border_radius=BORDER_RADIUS)
        
        # Add subtle inner shadow for depth
        shadow_color = tuple(max(0, c-15) for c in theme['felt'])
        shadow_rect = board_rect.inflate(-4, -4)
        pg.draw.rect(self.screen, shadow_color, shadow_rect, 2, border_radius=BORDER_RADIUS-2)
        ...
        for i in range(1, self.board.size):  # Skip outer border lines
            ...
            pg.draw.line(self.screen, grid_color, (x, board_rect.top + 2), (x, board_rect.bottom - 2), line_width)
            pg.draw.line(self.screen, grid_color, (board_rect.left + 2, y), (board_rect.right - 2, y), line_width)
    ...
    def draw_game_pieces(self, board_rect, cell):
        ...
        for r in range(self.board.size):
            for c in range(self.board.size):
                if self.board.grid[r][c] != EMPTY:
                    rect = pg.Rect(board_rect.left + c*cell, board_rect.top + r*cell, cell, cell)
                    ...
                    for anim in self.ui.flip_animations:
                        if anim.row == r and anim.col == c:
                            ...
                            self.draw_flipping_disc(rect, anim.from_color, anim.to_color, progress)
                            animating = True
                            break
        ...
        self.ui.flip_animations = [anim for anim in self.ui.flip_animations 
    ...
    def draw_hints(self, board_rect, cell, legal_moves):
        ...
        for (r, c) in legal_moves.keys():
            ...
            # Outer glow
            pg.draw.circle(self.screen, (255, 235, 120, 60), rect.center, hint_radius + 2)
            # Main hint dot
            pg.draw.circle(self.screen, (255, 245, 140), rect.center, hint_radius)
            # Inner highlight
            pg.draw.circle(self.screen, (255, 255, 200), (rect.centerx - 1, rect.centery - 1), max(1, hint_radius//2))
    ...
    def draw_wood_bg(self):
        ...
        if self.wood_cache and self.wood_cache.get_size()==self.screen.get_size():
            self.screen.blit(self.wood_cache, (0,0)); return
        w,h=self.screen.get_size(); surf=pg.Surface((w,h))
        ...
        for _ in range(40):  # Fewer, more refined grain circles
            x=rng.randint(0,w); y=rng.randint(0,h); r=rng.randint(15,45)
        ...
        self.wood_cache=surf; self.screen.blit(surf,(0,0))
    ...
    def draw_inner_shadow(self, rect, radius):
        ...
        for i in range(INNER_SHADOW_LAYERS):
            ...
            pg.draw.rect(s,(0,0,0,a), pg.Rect(i,i,rect.width-2*i,rect.height-2*i), border_radius=radius)
    ...
    def _create_textured_disc(self, radius, color):
        ...
        if color == BLACK:
            ...
            for ring in range(1, 4):
                ...
                pg.draw.circle(surf, pattern_color, (center_x, center_y), int(ring_radius), 1)
        ...
        if color == BLACK:
            ...
            pg.draw.arc(surf, (60, 60, 60), (center_x-radius, center_y-radius, radius*2, radius*2), 
                       math.pi, math.pi*1.5, 1)
        else:
            # Soft shadow on bottom-right
            pg.draw.arc(surf, (200, 200, 200), (center_x-radius, center_y-radius, radius*2, radius*2), 
                       0, math.pi*0.5, 1)
    ...
    def draw_flipping_disc(self, rect, from_color, to_color, progress):
        ...
        if progress < 0.5:
            ...
            if cache_key not in self.disc_cache:
                self.disc_cache[cache_key] = self._create_textured_disc(scaled_r, from_color)
            ...
            if cache_key not in self.disc_cache:
                self.disc_cache[cache_key] = self._create_textured_disc(scaled_r, to_color)
    ...
    def draw_move_history(self, history_rect, theme):
        ...
        title_rect = title_text.get_rect(centerx=history_rect.centerx, top=history_rect.top + 10)
        self.screen.blit(title_text, title_rect)
        
        # History content area
        content_rect = pg.Rect(history_rect.left + 10, title_rect.bottom + 10,
                              history_rect.width - 20, history_rect.height - title_rect.height - 30)
        
        if not self.ui.move_history:
            # No moves yet
            no_moves_text = pg.font.Font(None, 18).render("No moves yet", True, theme['text'])
        ...
        start_idx = max(0, len(self.ui.move_history) - visible_lines + self.ui.history_scroll)
    ...
    def draw_statistics(self, stats_rect, theme):
        ...
        title_rect = title_text.get_rect(centerx=stats_rect.centerx, top=stats_rect.top + 10)
        self.screen.blit(title_text, title_rect)
        
        # Content area
        content_rect = pg.Rect(stats_rect.left + 10, title_rect.bottom + 10,
                              stats_rect.width - 20, stats_rect.height - title_rect.height - 30)
        
        if not self.settings.stats or self.settings.stats.games_played == 0:
            # No games yet
            no_games_text = pg.font.Font(None, 18).render("No games played yet", True, theme['text'])
        ...
        for result in stats.recent_results[-5:]:  # Show last 5 games
            ...
            # Format result
            winner_text = "Win" if result.winner == result.human_color else "Loss" if result.winner == result.ai_color else "Tie"
            ...
            color = theme['accent'] if result.winner == result.human_color else theme['danger'] if result.winner == result.ai_color else theme['text']
    ...
    def draw_tutorial(self, tutorial_rect, theme):
        ...
        title_text = title_font.render(f"Strategy Tip {self.ui.tutorial.current_step + 1}", True, theme['accent'])
        title_rect = title_text.get_rect(centerx=tutorial_rect.centerx, top=tutorial_rect.top + 10)
        self.screen.blit(title_text, title_rect)
        
        # Step title
        step_font = pg.font.Font(None, 18)
        step_title = step_font.render(current_step.title, True, theme['text'])
        step_rect = step_title.get_rect(centerx=tutorial_rect.centerx, top=title_rect.bottom + 10)
        ...
        # Navigation hint
        nav_text = desc_font.render("Press T for next tip, ESC to close", True, tuple(c//2 for c in theme['text']))
        nav_rect = nav_text.get_rect(centerx=tutorial_rect.centerx, bottom=tutorial_rect.bottom - 5)
    ...
    def draw_analytics(self, analytics_rect, theme):
        ...
        title_rect = title_text.get_rect(centerx=analytics_rect.centerx, top=analytics_rect.top + 10)
        self.screen.blit(title_text, title_rect)
        ...
        if self.ui.analyzer.move_history:
            ...
            for analysis in recent_moves:
                ...
                # Quality indicator
                quality_surf = font_small.render(f"({analysis.move_quality})", True, quality_color)
                self.screen.blit(quality_surf, (analytics_rect.left + 100, content_y))
                
                # Pieces captured
                pieces_surf = font_small.render(f"+{analysis.pieces_captured}", True, theme['text'])
                self.screen.blit(pieces_surf, (analytics_rect.left + 160, content_y))
                
                content_y += line_height
        else:
            no_data_text = pg.font.Font(None, 16).render("No move data yet", True, theme['text'])
            no_data_rect = no_data_text.get_rect(center=(analytics_rect.centerx, analytics_rect.centery))
    ...
    def play(self,r,c):
        ...
        mv=legal.get((r,c))
        if not mv:
            self.ui.status="Illegal move"; self.sfx.play('bad'); return False
        t=pg.time.get_ticks()/1000.0
        
        # Analyze the move before making it
        move_analysis = self.gameplay_analyzer.analyze_move(self.board, mv, self.board.to_move)
        ...
        for (rr,cc) in mv.flips:
            ...
            self.ui.flip_animations.append(FlipAnimation(
                row=rr, col=cc, start_time=t, 
        ...
        self.sfx.play('place'); self.sfx.play('flip')
    ...
    def maybe_pass(self):
        if not self.board.legal_moves(self.board.to_move):
            self.board.pass_turn(); self.ui.status=f"{NAME[OPP[self.board.to_move]]} passes"; self.sfx.play('pass')

    def ai_turn_if_needed(self):
        if (self.board.to_move==BLACK and self.settings.ai_black) or (self.board.to_move==WHITE and self.settings.ai_white):
    ...
    def spawn_confetti(self,r,c):
        board_rect,_,cell=self.layout(); x=board_rect.left+c*cell+cell/2; y=board_rect.top+r*cell+cell/2
        for _ in range(CONFETTI_COUNT):
            ang=random.random()*2*math.pi; sp=CONFETTI_MIN_SPEED+random.random()*(CONFETTI_MAX_SPEED-CONFETTI_MIN_SPEED)
            vx=math.cos(ang)*sp; vy=math.sin(ang)*sp
            life=CONFETTI_MIN_LIFE+random.random()*(CONFETTI_MAX_LIFE-CONFETTI_MIN_LIFE)
            particle = Particle(x=x, y=y, vx=vx, vy=vy, life=life, age=0.0, color_id=random.randint(0,2))
    ...
    # Button handlers
    def on_undo(self):
        if not self.board.undo(): self.ui.status="Nothing to undo"
    def on_redo(self):
        if not self.board.redo(): self.ui.status="Nothing to redo"
    def on_toggle_hints(self):
        self.settings.hints=not self.settings.hints; self.settings.save()
    def on_toggle_ai_black(self):
        self.settings.ai_black=not self.settings.ai_black; self.settings.save()
    def on_toggle_ai_white(self):
        self.settings.ai_white=not self.settings.ai_white; self.settings.save()
    def on_cycle_depth(self):
        """Cycle AI difficulty level with proper validation"""
        self.settings.ai_depth = 1 + (self.settings.ai_depth % 6)
        # Ensure depth is within valid range
        self.settings.ai_depth = max(1, min(6, self.settings.ai_depth))
        try:
            self.ai = AI(max_depth=self.settings.ai_depth)
            self.settings.save()
            self.ui.status = f"AI difficulty set to level {self.settings.ai_depth}"
        except Exception as e:
    ...
    def on_cycle_board_size(self):
        ...
        current_index = size_options.index(self.board.size) if self.board.size in size_options else 2  # Default to 8x8
    ...
    def on_cycle_theme(self):
        ...
        current_index = theme_names.index(self.settings.theme) if self.settings.theme in theme_names else 0
        new_index = (current_index + 1) % len(theme_names)
        self.settings.theme = theme_names[new_index]
        
        # Clear caches to refresh with new theme colors
        self.wood_cache = None
        self.clear_disc_cache()
        self.settings.save()
        self.ui.status = f"Theme changed to {THEMES[self.settings.theme]['display']}"
    
    def on_toggle_sound(self):
        self.settings.sound=not self.settings.sound; self.sfx.enabled=self.settings.sound; self.settings.save()
    ...
    def on_new(self):
        ...
        self.board=Board(size=self.board.size); self.ui.last_move=None; self.ui.flip_animations.clear(); self.ui.move_history.clear(); self.ui.history_scroll=0; self.ui.game_start_time=time.time(); self.ui.game_end_time=None
    ...
    def on_save(self):
        ts=time.strftime("%Y%m%d-%H%M%S"); fn=self.ui.last_save or f"reversi-{ts}.json"
        try:
            ...
            self.ui.last_save=fn; self.ui.status=f"Saved to {fn}"
    ...
    def on_load(self):
        try:
            ...
            cands=[x for x in os.listdir('.') if x.endswith('.json') and os.path.isfile(x)]
            if not cands: 
    ...
    def on_make_desktop(self):
        try:
            path=os.path.expanduser("~/.local/share/applications/reversi-deluxe.desktop")
...
"""
            with open(path,'w', encoding='utf-8') as f: f.write(desktop)
            # Make the desktop file executable
            os.chmod(path, 0o755)
            self.ui.status=f"Created launcher at {path} (may need to log out/in to appear in menu)"
        except PermissionError:
        ...
        while True:
            ...
            for ev in pg.event.get():
                ...
                if ev.type==pg.MOUSEBUTTONDOWN and ev.button==1:
                    board_rect,_,cell=self.layout(); rc=self.xy_to_rc(ev.pos, board_rect, cell)
                    if rc and not self.board.game_over():
                        # Only allow human moves if current player is human
                        current_is_ai = (self.board.to_move==BLACK and self.settings.ai_black) or (self.board.to_move==WHITE and self.settings.ai_white)
                        if not current_is_ai:
                            if self.play(*rc): 
                                self.maybe_pass()
                                ai_timer = 0  # Reset AI timer after human move
                    else:
                        # Check move analysis window first (it should be on top)
                        if not self.move_analysis.handle_click(ev.pos):
                            self.menu_system.handle_click(ev.pos)
                        
                # Handle mouse wheel for scrolling in analysis
                if ev.type == pg.MOUSEWHEEL:
                    if self.move_analysis.active:
                        self.move_analysis.handle_scroll(-ev.y)  # Negative for natural scrolling
                    elif self.game_analysis.active:
                        self.game_analysis.handle_scroll(-ev.y)  # Negative for natural scrolling
                if ev.type==pg.KEYDOWN:
                    if ev.key==pg.K_q: return
                    elif ev.key==pg.K_u: self.on_undo()
                    elif ev.key==pg.K_r: self.on_redo()
                    elif ev.key==pg.K_h: self.on_toggle_hints()
                    elif ev.key==pg.K_a:
                        if self.board.to_move==BLACK: self.on_toggle_ai_black()
                        else: self.on_toggle_ai_white()
                    elif ev.key==pg.K_d: self.on_cycle_depth()
                    elif ev.key==pg.K_n: self.on_new()
                    elif ev.key==pg.K_s: self.on_save()
                    elif ev.key==pg.K_l: self.on_load()
                    elif ev.key==pg.K_m: self.on_toggle_sound()
                    elif ev.key==pg.K_g: 
                        if self.board.game_over():
                            if self.game_analysis.active:
                                self.game_analysis.hide_analysis()
                                ...
                                self.game_analysis.show_analysis()
                    elif ev.key==pg.K_v: self.on_toggle_move_analysis()
                    elif ev.key==pg.K_t: 
                        if self.ui.tutorial.active:
...
def main(argv:List[str]):
    ...
    size=settings.board_size  # Use saved board size as default
    
    if len(argv)>=2:
        try:
            size=int(argv[1]); assert size>=4 and size%2==0 and size<=16
        except (ValueError, AssertionError):
            print("Usage: reversi.py [even board size 4-16, default from settings]", file=sys.stderr); return 2
    
    board=Board(size=size)
    Game(board, settings).run()
    settings.save(); return 0
